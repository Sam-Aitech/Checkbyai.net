...
| 2 | Durable document key across instances | `02-storage-proof.md`: DocumentStore (local/S3), key-only jobs, orphan purge | R2 EU bucket + scoped token record, two-host run, restart-mid-job, empty bucket | ⏳ live run |
...
- Staging provenance (required for Gates 3–6 — EXPLAIN/load results are only
  ...
  - [ ] Seed output (status distribution table from the seed run): …
  - [ ] Staging URL: …
