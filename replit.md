...
- **Tiered Alert System**: Centralized in `server/utils/tierConfig.ts`. SubscriptionStatus mapping: `'pro'` → Unlimited tier (legacy naming). FREE=next morning 8am UTC, STARTER=same-day 6pm UTC, PRO/UNLIMITED/ENTERPRISE=immediate. Channel gating: WhatsApp requires Pro+, SMS requires Unlimited+. Hourly cron processes delayed notification queue.
