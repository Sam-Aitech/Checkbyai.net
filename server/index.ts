...
async function applyDataFixbacks() {
  ...
  try {
    ...
    // ── job_alert_preferences guard ──────────────────────────────────────────
    // Migration 0002 defines this table but drizzle-kit migrate is not always
    // run before the dev server starts.  This idempotent DDL ensures the table
    // is always present so the nightly JobAlertJob cron never fails with
    // "relation job_alert_preferences does not exist".
    try {
      await client.query(`
      ...
      `);
      await client.query(`CREATE INDEX IF NOT EXISTS "idx_job_prefs_user_id" ON "job_alert_preferences"("user_id")`);
      await client.query(`CREATE INDEX IF NOT EXISTS "idx_job_prefs_fingerprint" ON "job_alert_preferences"("fingerprint") WHERE enabled = TRUE`);
    } catch (err) {
      logger.error({ err }, "Failed to ensure job_alert_preferences table exists at startup");
    }
