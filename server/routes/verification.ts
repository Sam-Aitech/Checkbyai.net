...
export function registerVerificationRoutes(app: Express): void {
  ...
  app.post('/api/verify', verifyLimiter, upload.single('file'), asyncHandler(async (req: any, res) => {
    ...
    // Everything below reads/uses safeFilePath — the local temp file is pure
    // per-request scratch space (never touched by anything outside this
    // process), so one guaranteed cleanup point here replaces what used to
    // be six manually-scattered unlink calls, and — unlike the old code —
    // also covers assertSafeUploadFilename() throwing, which previously left
    // the file orphaned with no cleanup path at all.
    try {
      ...
      if (priorAdminFlag) {
        ...
        if (shouldQueue) {
          ...
          // of processing the new upload.
          const jobId = `verify-${documentHash.slice(0, 16)}-${userId}-${crypto.randomBytes(4).toString('hex')}`;
          try {
            // The worker may run as a separate, independently-scaled process
