...
### Fixed
- **Alert Pass pricing cards stuck on "Coming soon" live**: `GET /api/packages` was returning an empty `packages` array in production — the `alert_annual`/`alert_annual_pro`/`cos_check_single` Stripe products referenced in a prior CHANGELOG entry as "created manually" either never actually existed in the live Stripe account/mode, or existed in the wrong mode. `usePackagePrices().getPriceId()` had nothing to resolve, so `Pricing.tsx`'s annual plan cards (gated on `available={!!getPriceId(...)}`) rendered "Coming soon" instead of a working checkout button. `server/scripts/seedProducts.ts` (now idempotent, using `STRIPE_SECRET_KEY` directly) creates these three products/prices with the correct `metadata.packageType` — run via new `npm run seed:products`. Documented as a required first-run step in `docs/RUNBOOK.md`. Does not touch the legacy Starter/Pro/Unlimited/Master or monthly Notification plans — those sell through separate hardcoded Stripe Payment Links and were unaffected by this bug (their buttons never depended on `GET /api/packages`).
- **Payment-plan checkout hardening (Alert Pass / COS Check)**: `POST /api/checkout/credits` (`server/routes/billing.ts`) accepted a client-supplied `priceId` and `packageType` with nothing cross-checking that the price actually belonged to that package before entitlement was later granted keyed only on `packageType` — a user could submit a cheap price alongside an expensive `packageType` and receive full entitlement underpaid. Now retrieves the Stripe price (expanded to its product) and validates `metadata.packageType` (checked on the price, falling back to the product — matching how `usePackagePrices()` already resolves price IDs client-side) matches the client's claim before creating the Checkout Session; mismatches 400 instead of silently succeeding. Only `alert_annual`, `alert_annual_pro`, and `cos_check_single` route through this endpoint today; `starter`/`pro`/`unlimited`/`notification_starter`/`notification_pro` use the separate signed-Payment-Link flow and are unaffected.
- `STRIPE_WEBHOOK_SECRET` was effectively required (the webhook 500s on every event without it, so no paid entitlement is ever granted via webhook) but was treated as optional at boot (`server/index.ts` only warned) and documented as "Optional" in `.env.example`. Moved into the existing `REQUIRED_ENV_VARS` fail-fast check and corrected the `.env.example` wording.
- `server/scripts/seedProducts.ts` used a different Stripe credential path (the Replit connector, via `getUncachableStripeClient()`) than every other Stripe call in the app (`process.env.STRIPE_SECRET_KEY`), and only seeds 4 of the 10 live `packageType`s — the other 6 were created manually via the Stripe Dashboard. Switched it to `STRIPE_SECRET_KEY` for consistency and marked it deprecated/legacy-only rather than extending it, to avoid risking duplicate Stripe products.
- Removed dead route `POST /api/create-subscription` (`billing.ts`) — zero callers anywhere in the app; superseded by `/api/checkout/credits`. Removed unused `@stripe/stripe-js` / `@stripe/react-stripe-js` dependencies — no import anywhere in `client/src` since the Elements-based `SubscriptionModal.tsx` was deleted in a prior session; every live checkout path is a server-generated redirect.

### Changed
- **Full product separation — Alert Pass vs. COS Check**: closed a bidirectional entitlement leak between the two products. `server/routes/verification.ts`'s `/api/verify` gate previously treated `subscriptionStatus === 'pro'/'unlimited'` as proof of COS entitlement (`hasPaidPlanWithCos`), so any Alert Pass Pro or Pro Annual subscriber — a monitoring purchase — got unrestricted COS verification access they never paid for. Replaced with the actual COS-specific signals already in the schema: `cosCheckSubscription`, `cosCheckApproved`, admin role, `subscriptionStatus === 'enterprise'` (never auto-assigned), and held `credits` (audited across every credit-granting call site: minted only by `cos_check_single`, COS `starter`/`pro` packages, and the legitimate `notification_pro` monthly +5 bundle — never by `alert_annual`/`alert_annual_pro`/`notification_starter`). In the reverse direction, `server/routes/billing.ts`'s `applyPackageGrant` for the COS one-time credit packages (`starter` £24.99/50 checks, `pro` £39.99/100 checks) was also writing `subscriptionStatus: 'starter'/'pro'` into the same column Alert-Pass tier resolution (`shared/planTiers.ts`) reads — so a COS-only buyer silently got full Alert-Pass-Pro monitoring perks (job alerts, extra watch slots, SMS/WhatsApp channels) they never bought. Removed those `subscriptionStatus` writes for the two COS credit packages; COS Unlimited (£99.99, grants zero credits) now also sets `cosCheckSubscription: true` so its COS access survives independent of the (accepted, unchanged) Alert-Pass-Unlimited perks it bundles. Deleted `server/services/verificationService.ts` and `server/services/subscriptionService.ts` — stale duplicate copies of the same buggy gate/subscription logic with zero remaining callers anywhere in the codebase. Added `isPastDue()` to `shared/planTiers.ts` and wired it into `useAuth()` so a `past_due` subscriber sees explicit "payment failed" messaging instead of a plain "Free" label, without touching `resolveTier()`'s existing fallback behavior.
- **Alert-timing marketing accuracy**: discovered the automated notification pipeline (`server/services/consolidatedNotificationEngine.ts`, driven by the `CONSOLIDATED_NOTIFICATIONS` cron at 07:00/19:00 UTC) delivers every tier's alerts on the same twice-daily digest schedule with zero timing branching by tier — the tier-aware "immediate" dispatch path (`notificationDispatcher.ts`'s `notifyAffectedUsers`, honoring `TierConfig.alertTiming`) is only ever invoked from a manual admin-replay route, never the live pipeline. Every surface that claimed "Immediate alerts" for Pro/Unlimited/Enterprise (`Pricing.tsx`'s JSON-LD, `SponsorDashboard.tsx`, `billing.ts`'s confirmation email, and several other pages/components found via repo-wide audit) now uses the canonical `ALERT_TIMING_SHORT`/`ALERT_TIMING_COPY` constants already exported from `shared/planTiers.ts` ("Twice-daily alerts" / "Same-day digest at 18:00 UTC" for Starter) instead of overclaiming instant delivery.
- **Alert Pass repositioned as the primary product; COS Check fully separated as a secondary one**: `Pricing.tsx` now leads with the annual Alert Pass plans (`alert_annual` £9.99/yr, `alert_annual_pro` £19.99/yr flagship, now marketing its sponsored-job-alerts capability explicitly), with legacy monthly Notification Engine plans demoted to secondary; existing monthly subscribers are unaffected. `CheckoutSuccess.tsx` no longer auto-redirects to the dashboard after 2 seconds and branches receipt copy by exact `packageType` instead of one binary flag. The homepage (`HeroSection.tsx`) no longer sells COS verification as a co-equal product — the dedicated CoS section was replaced with a sponsored-job-alerts marketing section, and "Verify CoS" was demoted from a top-level nav item into the Resources dropdown across all three nav definitions (`PageLayout.tsx`, `HeroSection.tsx`'s hero nav, homepage). Fixed the homepage's static JSON-LD (`client/index.html`) and title/meta tags, which were 100% COS-branded regardless of all the sponsor-first copy already written elsewhere — this is what search engines and social-share crawlers actually saw. COS remains fully functional and reachable at `/cos-pricing`, `/dashboard`, `/verify-cos`, and via the demoted nav links — nothing was deleted, only de-prioritized on the primary Alert-Pass surfaces.
- **`client/src/pages/ProDashboard.tsx` (1191-line monolith) split into `client/src/pages/pro-dashboard/`**: `index.tsx` (Shell — nav, mobile bottom nav limited to Home/Monitor/Jobs/Alerts at 44px+ touch targets, auth guard, `prefers-reduced-motion` handling), `Overview.tsx` (protection-status-first: empty/clear/attention/critical semantics, never labels a zero-watch user "All clear", first-run onboarding with no COS mention, past-due banner, "Last change detected" — never a fabricated "last checked" timestamp), `Monitor.tsx`, new `Jobs.tsx` (surfaces the existing `/api/job-alert-preferences` per-company toggle — previously only reachable from `SponsorMonitor.tsx`, invisible from the dashboard), `Alerts.tsx` (combines sponsor alerts, job alerts, and delivery-channel preferences), `History.tsx`, `Support.tsx`, and new `Account.tsx` (Profile/Plan & Usage/Billing/Security/Support, including the demoted "Verify a CoS" link — real-gated on `cosCheckSubscription`/`cosCheckApproved`/admin/credits, matching the server's actual entitlement check, linking out to the existing `/dashboard` COS page rather than duplicating its upload UI). Each sub-page is now a real, URL-addressable route (`/pro-dashboard/monitor`, `/jobs`, `/alerts`, `/history`, `/support`, `/account`) registered flatly in `App.tsx`, so direct load, refresh, and browser back/forward all work — the old version tracked its active tab in component state only. No new backend endpoints or DB migration; every page composes existing APIs (new `useAccountSummary()` hook composes `useAuth()` + `/api/watches` + `/api/sponsor-changes` + `/api/credits`).
- **Pricing restructure — annual alert passes + pay-per-use CoS checks**: Added `alert_annual` (£9.99/yr, 1 company), `alert_annual_pro` (£19.99/yr, up to 5 companies), and `cos_check_single` (£4.99, one-time) packageTypes end to end through `server/routes/billing.ts` (webhook grant, `/api/checkout/verify`, `/api/checkout/credits`). New Stripe products created and tagged with `metadata.packageType` so `/api/packages` resolves their price IDs at runtime, no hardcoded Payment Links needed for these three. **Correction (2026-09-13):** the products described here did not actually exist live in production — see the "Alert Pass pricing cards stuck on 'Coming soon' live" entry above; `server/scripts/seedProducts.ts` now creates them. Existing monthly Notification Engine (Starter/Pro) and CoS credit plans (`starter`/`pro`/`unlimited`) kept unchanged for existing subscribers, no grandfathering migration. Annual passes are modeled as a 12-month Stripe subscription with `cancel_at` set via a follow-up `stripe.subscriptions.update()` call (Checkout's `subscription_data` has no `cancel_at` field), so the existing `customer.subscription.deleted` webhook auto-downgrades the user to `free` at expiry without a new cron job. `CosPricing.tsx` restructured: Master Package (human review) folded into a new Enterprise contact-sales card, `cos_check_single` added as the primary pay-per-use option, Unlimited/Starter/Pro repositioned as secondary.
...
- **Frontend craft audit remediation (P0+P1)**: `server/ssr/renderLanding.ts` migrated from hardcoded hex (`#2563eb` et al) to theme CSS vars with pill CTAs and `80rem/1.5rem` containers matching the client; `NightlyStatsBar` overlap hack (`-mt-16`) removed and landing containers unified on `max-w-7xl px-6 md:px-8`; status badges raised to `text-xs` minimum with wider tracking (`SponsorMonitor.tsx`, `SponsorDirectory.tsx`); `badge-live` contrast fixed (`#047857`); `ui/button.tsx` moved to global pill radius with scoped easing + press scale; `ui/input.tsx` pill with softer placeholder and 44px touch floor; modal uses a `0.2s` tween instead of a bouncy spring with a 44px close target; `FeatureCard` tiles and stagger, card/icon/OTP transitions, and the `float` keyframe (no more 180° spin) all brought onto one `cubic-bezier(0.16,1,0.3,1)` motion language. Touched files lint clean.
- **Frontend visual-system remediation (full audit)**: `client/src/index.css` is now the single token source — surfaces, text, borders, AA-verified status tokens (light/dark), opaque hero text tokens, radius tiers (4/8/12/16/24/pill), motion tokens (120/180/260ms + one easing curve), container primitives (wide 1200/content 960/reading 720), semantic type roles (`display/h1/h2/h3/body/body-sm/label/caption`), soft status badges, dark-surface hero utilities, and dashboard primitives. `theme-card` is static-only with a new `theme-card-interactive` variant; buttons use semantic geometry (brand pill, control/compact radii); inputs/selects/textarea moved to 8px control radius with solid AA placeholders and 44px touch floors; touch rule narrowed to coarse pointers. Hero text migrated off translucent white to opaque tokens (measured 5.8–17:1), search has a real focus-visible ring, switch lost `scale-90` with a 44px label target, and status badges/CTAs moved to AA shades (emerald/red/blue-600+, amber-700). `AnimatedBackground` dropped its 5 rotating blurred shapes and 30→12 particles; `transition-all` eliminated from the frontend; dead CSS (`pulse-glow`, `shimmer`, `urgency-dot`, `glass-card`) removed. `shared/landingCopy.ts` keeps SSR and hydrated landing copy identical, and the SSR landing now mirrors client section order inside the real gradient (verified via headless-Chromium screenshots at 390/1280px, no overflow). `ProDashboard.tsx`/`CompanyIntelligencePanel.tsx` moved repeated inline styles into `dash-*` classes with theme-aware status colors; custom icons use CSS-variable gradient stops; login keeps a single brand mark. Full `npm run lint` passes with zero errors, `tsc` clean, 475 unit tests pass, `vite build` succeeds.
